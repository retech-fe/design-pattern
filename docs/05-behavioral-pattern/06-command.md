...
命令模式（Command Pattern）又称事务模式，将请求封装成对象，将命令的**发送者**和**接受者**解耦。本质上是对**方法调用的封装**。
...
命令模式和职责链模式可以结合使用，比如具体命令的执行，就可以引入职责链模式，让命令由**职责链**中合适的处理者执行。

### 8.2 命令模式与组合模式

命令模式和组合模式可以结合使用，比如不同的命令可以使用组合模式的方法形成一个**宏命令**，执行完一个命令之后，再继续执行其子命令。
